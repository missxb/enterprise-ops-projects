...
log_info "=== Redis Cluster生产级部署(3主3从) ==="
log_info "节点: ${NODES}"
...
if [ "${node_count}" -lt 6 ]; then
  log_error "  ❌ 节点数不足(需6个,当前${node_count}个)"
  errors=$((errors+1))
fi
...
EOF
  log_ok "  ✅ ${node} Redis已安装"
done

# Step 2: 创建Cluster
log_info ""
log_info ">>> Step 2: 创建Redis Cluster"
