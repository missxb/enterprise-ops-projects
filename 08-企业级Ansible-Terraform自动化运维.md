...
> **⚠️ 首次使用需先执行bootstrap**
> deploy用户需要先创建，运行以下命令以root身份初始化:
> ```bash
> ansible-playbook -i inventory/hosts playbooks/bootstrap.yml --user root
> ```
...
- name: 基础服务器配置
  ...
  tasks:
    ...
    - name: 关闭SELinux
      selinux:
        state: permissive
    # > **等保冲突**: SELinux permissive是K8s节点的推荐状态(容器运行时需要)。
    # > 生产建议:
    # > - K8s节点: 保持permissive(容器运行时需要,允许通过审计日志检查SELinux违规)
    # > - 非K8s节点: 使用enforcing(等保三级要求)
    # > - 完全禁用(disabled)不推荐: 违反等保要求且无法审计
    ...
    - name: 关闭防火墙
      systemd:
        ...
        state: stopped
        enabled: no
    # > **等保冲突**: 关闭firewalld违反等保要求。生产建议:
    # > - K8s节点: 关闭firewalld但启用iptables白名单(容器网络需要iptables)
    # >   使用K8s推荐的iptables规则: 允许10250(api-server), 10255(heapster),
    # >   30000-32767(NodePort), 179(calico/bgp), 5473(calico/vxlan)等
    # > - 非K8s节点: 启用firewalld并配置白名单规则
    # > - 网络策略: 即使使用iptables白名单,仍需配合K8s NetworkPolicy实现Pod级隔离
    ...
    # > **SSH安全加固**:
    # > - ClientAliveCountMax 3: 客户端3次无响应后断开(3*300=900秒超时)
    # > - MaxSessions 10: 限制每个连接的最大会话数,防止资源耗尽
    # > - LoginGraceTime 30: 登录认证超时30秒,减少暴力破解窗口
    # > - Banner /etc/ssh/banner: 登录前显示安全警告,符合等保要求
      notify: restart sshd
    ...
    # > **密钥管理**: 使用专用部署密钥(deploy_key)而非默认SSH密钥(id_rsa)。
    # > 确保ansible.cfg中ansible_ssh_private_key_file指向deploy_key私钥。
...
resource "alicloud_db_instance" "mysql" {
  ...
  security_ips         = ["10.0.0.0/12"]  # 与VPC CIDR一致
  security_group_id    = alicloud_security_group.k8s.id
  
  db_instance_storage_type = "cloud_essd"
  db_instance_disk_category = "cloud_essd"
  
  # RDS备份策略：每日全量备份，保留30天，7天内支持按时间点恢复
  backup_period    = "Monday,Tuesday,Wednesday,Thursday,Friday,Saturday,Sunday"
  backup_time      = "02:00Z-03:00Z"  # 低峰期执行
  backup_retention_period = 30  # 备份保留30天
  log_backup               = true  # 开启日志备份，支持精确恢复
  log_backup_retention_period = 7  # 日志备份保留7天
...
variable "db_password" {
  description = "RDS MySQL root密码"
  type        = string
  sensitive   = true
  # 生产环境通过 TF_VAR_db_password 环境变量传入，切勿硬编码
}
...
variable "redis_password" {
  ...
  type        = string
  sensitive   = true
  # 生产环境通过 TF_VAR_redis_password 环境变量传入，切勿硬编码
}
...
destroy:
	@echo "⚠️  即将销毁所有基础设施资源，此操作不可逆！"
	@read -p "请输入 'destroy' 确认销毁: " CONFIRM && [ "$$CONFIRM" = "destroy" ] || (echo "已取消" && exit 1)
...
# 使用密码文件
ansible-playbook site.yml --vault-password-file=/etc/ansible/.vault_pass
# 生产环境必须确保密码文件权限正确
...
CACHE_DIR = os.path.join(os.path.expanduser('~'), '.ansible_inventory_cache')
CACHE_TTL = 300  # 缓存有效期(秒)
def get_cached_inventory():
    """读取缓存"""
    os.makedirs(CACHE_DIR, exist_ok=True)
    cache_file = os.path.join(CACHE_DIR, hashlib.md5(CMDB_URL.encode()).hexdigest())
...
def get_inventory():
    ...
    try:
        ...
        if CMDB_TOKEN:
            req.add_header('Authorization', f'Bearer {CMDB_TOKEN}')
...
vault_password_file = /etc/ansible/.vault_pass_prod
...
- name: Ansible执行监控
  ...
  tasks:
    ...
    - name: 记录开始时间
      uri:
        url: "{{ pushgateway_url }}/metrics/job/ansible"
        method: POST
        body: |
...
# 推送指标到Pushgateway
curl -X POST "$PUSHGATEWAY_URL/metrics/job/terraform" \
  --data-binary "terraform_apply_duration_seconds{environment=\"production\"} $DURATION"

# 记录变更资源数
CHANGE_COUNT=$(terraform plan -no-color 2>&1 | grep -c "to be")
curl -X POST "$PUSHGATEWAY_URL/metrics/job/terraform" \
  --data-binary "terraform_changes{environment=\"production\"} $CHANGE_COUNT"
```
