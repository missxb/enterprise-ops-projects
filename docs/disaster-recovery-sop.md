...
| 组件 | RPO(恢复点目标) | RTO(恢复时间目标) | 备份策略 |
|------|-----------------|-------------------|----------|
| K8s etcd | < 6小时 | < 30分钟 | 每6小时快照+OSS异地 |
| MySQL MGR | < 1秒 | < 5分钟 | MGR自动failover+Xtrabackup |
| Redis Cluster | < 1秒 | < 1分钟 | 主从同步+AOF |
| ELK/ES | < 1小时 | < 2小时 | Snapshot+ILM |
| Harbor | < 24小时 | < 1小时 | OSS镜像复制 |
